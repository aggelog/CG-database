...
void init3Dmodel()
{
	...
	glBufferData( GL_ARRAY_BUFFER,(sizeof(m->Positions[0]) * m->Positions.size()) + (sizeof(m->TexCoords[0]) * m->TexCoords.size()),NULL, GL_STATIC_DRAW );
	glBufferSubData( GL_ARRAY_BUFFER, 0, (sizeof(m->Positions[0]) * m->Positions.size()), &m->Positions[0] );
	glBufferSubData( GL_ARRAY_BUFFER, (sizeof(m->Positions[0]) * m->Positions.size()), (sizeof(m->TexCoords[0]) * m->TexCoords.size()), &m->TexCoords[0] );
	...
	glVertexAttribPointer( vTexCoord, 2, GL_FLOAT, GL_FALSE, 0,BUFFER_OFFSET((sizeof(m->Positions[0]) * m->Positions.size())) );
...
void display3Dmodel(glm::mat4 tsl,glm::vec3 positionv,glm::vec3 directionv,glm::vec3 upv,int mode)
{
	...
	AvgColor[0] = (Color0[0] + Color1[0]) / 2.0f;
	AvgColor[1] = (Color0[1] + Color1[1]) / 2.0f;
	AvgColor[2] = (Color0[2] + Color1[2]) / 2.0f;

	glUniform3fv(glGetUniformLocation(program3Dmodel,"Color0"),1,Color0);
	glUniform3fv(glGetUniformLocation(program3Dmodel,"Color1"),1,Color1);
	glUniform3fv(glGetUniformLocation(program3Dmodel,"AvgColor"),1,AvgColor);
	glUniform1f(glGetUniformLocation(program3Dmodel,"Frequency"),Frequency);
	...
	glUniform1i(glGetUniformLocation(program3Dmodel,"with_aliasing"),with_aliasing);
...
void initFrequClampControl()
{
	FrequClamp_Bar = TwNewBar("Frequency_Clamping_Control_Bar");

	TwDefine("Frequency_Clamping_Control_Bar label='FrequencyClamping Control TwBar' position='16 380' alpha=0 help='Use this bar to control the objects of the scene.' ");

	TwAddVarRW(FrequClamp_Bar, "Checkboard Color0", TW_TYPE_COLOR3F, &Color0, " label='Checkboard Color0' ");
	TwAddVarRW(FrequClamp_Bar, "Checkboard Color1", TW_TYPE_COLOR3F, &Color1, " label='Checkboard Color1' ");
	TwAddVarRW(FrequClamp_Bar, "Frequency", TW_TYPE_FLOAT, &Frequency, "min=0.1 max=100.0 step=0.1 label='Frequency' ");
}
...
int main (int argc, const char * argv[])
{
	...
	std::string vendor ="Title : Chapter-17   Vendor : " + stream1.str() + "   Renderer : " +stream2.str();
	...
	while (running) {
		...
		mat = glm::translate(mat,glm::vec3(3.0,0.0,0.0));

		display3Dmodel(mat,pos,direction,up,0);
