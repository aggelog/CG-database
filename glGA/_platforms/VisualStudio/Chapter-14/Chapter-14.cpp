...
float     GratingSpacing   = 16.0f;
float     SurfaceRoughness = 0.15f;
...
void create_Diffraction_bar()
{
	// Create a new tweak bar and change its label, position and transparency
	BumpWin = TwNewBar("Diffraction-Control");
    TwDefine(" Diffraction-Control label='Diffraction Control' position='16 380' alpha=0 help='Use this bar to control the diffraction effect.' ");

	TwAddVarRW(BumpWin, "LightPos", TW_TYPE_DIR3F, &light_position," label='Light Position'");

	TwAddVarRW(BumpWin, "GratingSpacing", TW_TYPE_FLOAT, &GratingSpacing," label='Grating Spacing' min=0.0");

	TwAddVarRW(BumpWin, "SurfaceRoughness", TW_TYPE_FLOAT, &SurfaceRoughness," label='Surface Roughness' min=0.0 max=10.0 step=0.01");

	TwAddVarRW(BumpWin, "HighlightColor", TW_TYPE_COLOR3F, &HighlightColor, " label='Highlight Color'");
...
void init3Dmodel()
{
	...
	//m->loadMesh("./Models/dvd/DVD_CD.obj");
	...
	glBufferData( GL_ARRAY_BUFFER,(sizeof(m->Positions[0]) * m->Positions.size()) + (sizeof(m->Normals[0]) * m->Normals.size()) + (sizeof(m->Tangents[0]) * m->Tangents.size()),NULL, GL_STATIC_DRAW );
	...
	glBufferSubData( GL_ARRAY_BUFFER,  (sizeof(m->Positions[0]) * m->Positions.size()) + (sizeof(m->Normals[0]) * m->Normals.size()),(sizeof(m->Tangents[0]) * m->Tangents.size()), &m->Tangents[0] );
	...
	GLuint vTangent = glGetAttribLocation( program3Dmodel, "MCTangent" );
	glEnableVertexAttribArray( vTangent );
	glVertexAttribPointer( vTangent, 3, GL_FLOAT, GL_FALSE, 0,BUFFER_OFFSET((sizeof(m->Positions[0]) * m->Positions.size()) + (sizeof(m->Normals[0]) * m->Normals.size())  ));
...
void display3Dmodel(glm::mat4 tsl,glm::mat4 lm,glm::vec3 positionv,glm::vec3 directionv,glm::vec3 upv)
{
	...
	glUniform4fv(glGetUniformLocation(program3Dmodel,"HighlightColor"),1,glm::value_ptr(HighlightColor));
	glUniform1f(glGetUniformLocation(program3Dmodel,"GratingSpacing"),GratingSpacing);
	glUniform1f(glGetUniformLocation(program3Dmodel,"SurfaceRoughness"),SurfaceRoughness);

	//Calculation of ModelView Matrix
	glm::mat4 model_mat = glm::mat4();
	model_mat =tsl * glm::scale(glm::mat4(),glm::vec3(2.5,2.5,2.5));
...
int main (int argc, const char * argv[])
{
	...
	std::string vendor ="Title : Chapter-14   Vendor : " + stream1.str() + "   Renderer : " +stream2.str();
