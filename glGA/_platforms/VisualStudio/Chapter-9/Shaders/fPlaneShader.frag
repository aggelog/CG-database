...
// GL_REPLACE computation
vec4 gl_replace()
{
	return texture(gSampler, TexCoord[0].xy);
}
...
{ 
	...
	for(int i = 0; i < 4; i++)
	{
		if(lights[i].active == 1 && lights[i].position.w == 0.0)
		{
			DirectionalLight(i,normal,amb,diff,spec);
			active++;
		}
		else if(lights[i].active == 1 && lights[i].spotCutoff == 180.0)
		{
