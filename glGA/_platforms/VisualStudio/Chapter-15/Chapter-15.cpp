...
void init3Dmodel()
{
	...
	glBufferData( GL_ARRAY_BUFFER,(sizeof(m->Positions[0]) * m->Positions.size()) + (sizeof(m->Normals[0]) * m->Normals.size()),NULL, GL_STATIC_DRAW );
...
void initNoiseControlBar()
{
	Noise_Bar = TwNewBar("Noise_Control_TwBar");

	TwDefine("Noise_Control_TwBar label='Noise Control TwBar' position='16 380' alpha=0 help='Use this bar to control the objects of the scene.' ");

	TwAddVarRW(Noise_Bar, "Sky Color", TW_TYPE_COLOR3F, &SkyColor, " label='Sky Color' ");
	TwAddVarRW(Noise_Bar, "Cloud Color", TW_TYPE_COLOR3F, &CloudColor, " label='Cloud Color' ");
	TwAddVarRW(Noise_Bar, "Scale Factor", TW_TYPE_FLOAT, &Scale, "min=-10.0 max=10.0 step=0.005 label='Scale Factor' ");
	TwAddVarRW(Noise_Bar, "Light Position", TW_TYPE_DIR3F, &LightPos, " label='Light position' ");
